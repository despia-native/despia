...
// always: `DespiaBase` used to be pure Swift over the platform's own libsqlite3,
// which made it a SECOND implementation of the store semantics - the 26 fixtures
// in OpenSource/Conformance/base ran on TypeScript and Kotlin but never on it,
// and sqlite-vec was absent from it entirely, because the system SQLite carries
// no vector extension and no amount of Swift can add one. `DespiaBaseCore` below
// is the closure: engine/src plus the vendored amalgamation and sqlite-vec,
// compiled as ONE clang target, with `DespiaBase` a marshaller over the eight
...
let engineDefines: [(String, String?)] = [
    // api.cpp reports this through despia_base_capabilities(); it is the
    // package's own VERSION file, which ClosedSource/scripts/ai_package_gate.rb
    // checks the rest of the package against.
    ("DESPIA_BASE_VERSION", "\"0.1.0\""),
    ("DESPIA_BASE_BUILDING", "1"),
...
let package = Package(
    ...
    products: [
        .library(name: "DespiaBase", targets: ["DespiaBase"]),
    ],
    targets: [
        ...
        .target(
            name: "DespiaBase",
            dependencies: ["DespiaBaseCore"],
            path: "bindings/swift/Sources/DespiaBase"
        ),
        // The corpus runner. A TEST target rather than an executable so
        // `swift test` on the mac lane is the whole gate, and it reads
        // OpenSource/Conformance/base from the tree rather than vendoring a copy:
        // one corpus, three runners, and no chance of a stale duplicate passing
        // while the shared fixtures moved on.
        .testTarget(
            name: "DespiaBaseConformanceTests",
            dependencies: ["DespiaBase"],
            path: "bindings/swift/Tests/DespiaBaseConformanceTests"
        ),
    ],
