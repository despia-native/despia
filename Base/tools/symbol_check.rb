...
#
#   ruby tools/symbol_check.rb [path/to/libdespia_base.so] [--nm <tool>]
#
# THE RULE (vendor/VERSIONS, engine/include/despia_base.h): libdespia_base
# exports the despia_base_* ABI and NOTHING else. In particular no sqlite3_*
# and no vec_*.
#
...
#
#   ruby tools/symbol_check.rb build/libdespia_base.so
#   ruby tools/symbol_check.rb bindings/kotlin/build/.../arm64-v8a/libdespia_base.so \
...
# The complete exported surface, from engine/include/despia_base.h. Adding a
...
EXPECTED = %w[
  despia_base_abi_version
  despia_base_call
  despia_base_capabilities
  despia_base_close
  despia_base_free
  despia_base_last_error
  despia_base_open
  despia_base_reopen
].freeze
...
library = args.shift || File.join(__dir__, "..", "build", "libdespia_base.so")
...
puts "[symbol_check] #{library}: #{exported.size} exported symbols, " \
     "exactly the despia_base_* ABI — 0 sqlite3_*, 0 vec_*."
