//
//  DespiaBase.swift - the Swift face of the C ABI.
//
...
//  OpenSource/Conformance/base never touched it, and sqlite-vec - the reason the
...
//
//    despia_base_abi_version · despia_base_capabilities · despia_base_free ·
//    despia_base_open · despia_base_close · despia_base_reopen ·
//    despia_base_call · despia_base_last_error
//
...
//  DespiaBaseCore, which compiles engine/src plus the vendored SQLite and
...
//
//    * `despia_base_call` returns a CALLER-owned pointer. It is parsed and freed
//      with `despia_base_free` in the same scope, through a `defer`, so an early
//      return cannot leak it.
//    * `despia_base_capabilities` returns a CALLER-owned pointer, freed the same
//      way.
//    * `despia_base_last_error` returns a HANDLE-owned pointer valid only until
//      the next call on that handle. It is COPIED immediately and never freed;
//      freeing it would be a double free.
//
//  THE THREADING CONTRACT (despia_base.h, and it is part of the ABI): one handle
...
//  SEMANTICS are pinned by OpenSource/Conformance/base, which
//  Tests/DespiaBaseConformanceTests runs against this very file.
//

import Foundation

#if canImport(DespiaBaseCore)
import DespiaBaseCore
#endif
...
public struct BaseError: Error, Sendable, Equatable {
    ...
    /// act on.
    static func from(_ raw: String?) -> BaseError {
        guard let raw, let parsed = Json.parseOrNull(raw) else {
            return BaseError("call_failed", "the store failed without reporting why")
        }
        return BaseError(parsed["code"].string("call_failed"),
...
public struct Embedding: Sendable, Equatable {
    ...
    public init(model: String, dimension: Int, revision: String? = nil) throws {
        guard !model.isEmpty else {
            throw BaseError("invalid_embedding", "an embedding names its model")
        }
        guard dimension >= 1, dimension <= Embedding.maxDimension else {
            throw BaseError("invalid_embedding",
...
///
/// The Kotlin twin is `com.despia.base.Store`; the name differs because this type
/// is the Swift package's namesake and its callers say `DespiaBase(directory:)`.
public final class DespiaBase {
    ...
    public init(directory: URL, readonly: Bool = false) throws {
        ...
        guard let opened = despia_base_open(Json.object(config).render()) else {
            // There is no handle to ask, so the failure is the GLOBAL one.
            throw DespiaBase.globalError()
        }
        handle = opened
    }
    ...
    public func crash() throws {
        let open = try alive()
        if despia_base_reopen(open) != 0 { throw lastError() }
    }
    ...
    public func call(_ action: String, _ args: Json = .object(JsonObject())) throws -> Json {
        ...
        // say answers `{}`.
        guard let raw = despia_base_call(open, action, args.render()) else {
            throw lastError()
        }
        defer { despia_base_free(UnsafeMutableRawPointer(raw)) }
        guard let value = Json.parseOrNull(DespiaBase.copy(raw)) else {
            throw BaseError("call_failed", "the store answered something that is not JSON")
        }
        return value
    }
    ...
    private func alive() throws -> UnsafeMutableRawPointer {
        guard let open = handle else {
            throw BaseError("closed", "this store was closed")
        }
        return open
    }
    ...
    public static var abiVersion: UInt32 { despia_base_abi_version() }
    ...
    public static func capabilities() throws -> Json {
        guard let raw = despia_base_capabilities() else {
            throw BaseError("call_failed", "the library reported no capabilities")
        }
        ...
        defer { despia_base_free(UnsafeMutableRawPointer(mutating: raw)) }
        guard let value = Json.parseOrNull(DespiaBase.copy(raw)) else {
            throw BaseError("call_failed", "the library reported capabilities that are not JSON")
        }
        return value
    }
    ...
    private func lastError() -> BaseError {
        guard let open = handle, let raw = despia_base_last_error(open) else {
            return BaseError("call_failed", "the store failed without reporting why")
        }
        return BaseError.from(DespiaBase.copy(raw))
    }

    /// The last GLOBAL (open-time) error. `nil` is what the header says to pass
    /// when there is no handle to ask.
    private static func globalError() -> BaseError {
        guard let raw = despia_base_last_error(nil) else {
            return BaseError("call_failed", "the store could not be opened and said nothing")
        }
        return BaseError.from(copy(raw))
    }
