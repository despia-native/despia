...
final class JsonTests: XCTestCase {
    ...
    func testAnEmbeddingRefusesWhatCannotBeTrue() {
        XCTAssertThrowsError(try Embedding(model: "", dimension: 4)) { error in
            XCTAssertEqual((error as? BaseError)?.code, "invalid_embedding")
        }
    ...
    func testAnErrorsWireFormIsWhatAFixtureCompares() {
        let error = BaseError("embedding_mismatch", "the index was built by another embedder",
        ...
        XCTAssertEqual(BaseError("closed").wire.render(), "{\"code\":\"closed\"}")
    }
}
