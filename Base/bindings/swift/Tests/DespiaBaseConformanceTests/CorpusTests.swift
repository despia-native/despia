//
//  CorpusTests.swift - the Swift runner for the Despia Base conformance corpus.
//
//  Copyright Despia. Licensed under the Apache License, Version 2.0.
//
//  The same files OpenSource/Base/conformance/run.ts and the Kotlin CorpusTest
//  read, driven through the C ABI. That is the entire reason this exists. Until
//  the Swift binding was rewritten onto despia_base.h it was a SECOND
...
//  without the vector backend (DESPIA_BASE_HAS_VEC=0) genuinely cannot run the
...
//       a BaseError where Kotlin's `require` throws IllegalArgumentException), so
...
final class CorpusTests: XCTestCase {
    ...
    /// vendored at `conformance/base`, while in the monorepo they live at
    /// `OpenSource/Conformance/base`.
    private func corpusRoot() -> URL? {
        ...
        for start in starts {
            ...
            while true {
                for candidate in ["OpenSource/Conformance/base", "conformance/base"] {
    ...
    private final class Harness {
        ...
        init(_ store: DespiaBase) { self.store = store }
    }
    ...
    private func callBase(_ h: Harness, _ action: String, _ args: Json) throws -> Json {
        ...
        default:
            throw BaseError("unknown_action", "no base action named \(action)")
        }
    }
        ...
        for (_, model) in testCase["mock"]["models"].fields.pairs {
            for turn in model["turns"].items {
                for step in turn.items {
                    ...
                    do {
                        if spec["fails"].bool() {
                            throw BaseError("tool_failed", "the tool failed mid-write")
                        }
    ...
    private func runCase(_ testCase: Json, _ fails: inout [String]) throws {
        let dir = URL(fileURLWithPath: NSTemporaryDirectory())
            .appendingPathComponent("despia-base-" + UUID().uuidString)
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }

        let store = try DespiaBase(directory: dir)
        defer { store.close() }
        let h = Harness(store)
    ...
    private func verify(_ expect: Json, _ h: Harness, _ store: DespiaBase,
    ...
    /// `despia_base_capabilities()` rather than from a list kept here, which is
    ...
    func testTheBaseCorpusPassesOnTheSwiftBinding() throws {
        guard let root = corpusRoot() else {
            XCTFail("the base corpus was not found — looked for OpenSource/Conformance/base " +
                    "and conformance/base above this file and above the working directory")
            return
        }

        let carried = Set(try DespiaBase.capabilities()["actions"].strings)
    ...
    func testTheSeamReportsWhatThisBuildCarries() throws {
        XCTAssertEqual(DespiaBase.abiVersion, 1)

        let caps = try DespiaBase.capabilities()
    ...
    func testAClosedStoreRefusesRatherThanReachingADanglingHandle() throws {
        ...
        XCTAssertThrowsError(try store.stats()) { error in
            XCTAssertEqual((error as? BaseError)?.code, "closed")
        }
    }
}
