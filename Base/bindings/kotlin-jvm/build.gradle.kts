// Despiabase - com.despia:base-jvm.
//
...
//    libdespia_base the phone lanes link, which is the point - a binding that
...
val cmakeConfigure by tasks.registering(Exec::class) {
    ...
    commandLine(
        ...
        "-DDESPIA_BASE_BUILD_TESTS=OFF", "-DDESPIA_BASE_VEC=ON",
...
val cmakeBuild by tasks.registering(Exec::class) {
    ...
    outputs.file(cmakeDir.map { it.file("libdespia_base.$sharedSuffix") })
...
val stageEngine by tasks.registering(Copy::class) {
    ...
    from(cmakeDir.map { it.file("libdespia_base.$sharedSuffix") })
    into(nativeDir)
}
...
val buildJni by tasks.registering(Exec::class) {
    ...
    val shim = file("src/main/jni/despia_base_jni.cpp")
    doFirst { out.mkdirs() }
    inputs.file(shim)
    inputs.file(packageRoot.resolve("engine/include/despia_base.h"))
    outputs.file(File(out, "libdespia_base_jni.$sharedSuffix"))
    commandLine(
        buildList {
            ...
            add(shim.path)
            addAll(listOf("-L" + out.path, "-ldespia_base"))
            add("-Wl,-rpath,$loaderPath")
            addAll(listOf("-o", File(out, "libdespia_base_jni.$sharedSuffix").path))
        }
    )
}
