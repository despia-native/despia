// Store.kt - the typed Kotlin face over despia_base.h.
//
...
class Store(directory: File, readonly: Boolean = false) : AutoCloseable {
    ...
    init {
        ...
        if (handle == 0L) {
            ...
            throw BaseError.from(NativeStore.lastError(0L))
        }
    }
    ...
    fun crash() {
        alive()
        if (NativeStore.reopen(handle) != 0) throw BaseError.from(NativeStore.lastError(handle))
    }
    ...
    fun call(action: String, args: Json = Json.Obj(emptyMap())): Json {
        ...
        val raw = NativeStore.call(handle, action, args.render())
            ?: throw BaseError.from(NativeStore.lastError(handle))
        return Json.parse(raw)
    }

    private fun alive() {
        if (handle == 0L) throw BaseError("closed", "this store was closed")
    }
