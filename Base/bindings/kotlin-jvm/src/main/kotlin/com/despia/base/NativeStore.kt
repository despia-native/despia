...
// mapping is deliberately one-for-one with despia_base.h so that reading the
...
internal object NativeStore {
    ...
    init {
        ...
        // inspection.
        var loaded = runCatching { System.loadLibrary("despia_base_jni") }.isSuccess
        if (!loaded) {
            ...
            for (dir in candidates) {
                val lib = File(dir, System.mapLibraryName("despia_base_jni"))
        ...
        if (!loaded) {
            error("libdespia_base_jni was not found by name or at ${searched.joinToString(", ")}. " +
