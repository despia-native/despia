// despia_base_jni.cpp - the JVM's door to the Despia Base C ABI.
//
...
// despia_base.h, and this file is that seam for the JVM.
//
...
// (OpenSource/Base/tools/symbol_check.rb proves there are exactly eight). If the
...
//
// The threading story is short because the ABI's is: despia_base_call is
...
// one direction each way: the pointer from despia_base_call belongs to the
// caller and is freed here, and the pointer from despia_base_last_error belongs
...
Java_com_despia_base_NativeStore_abiVersion(JNIEnv*, jclass) {
  return static_cast<jint>(despia_base_abi_version());
}

JNIEXPORT jstring JNICALL
Java_com_despia_base_NativeStore_capabilities(JNIEnv* env, jclass) {
  const char* raw = despia_base_capabilities();
  if (!raw) return nullptr;
  jstring out = env->NewStringUTF(raw);
  despia_base_free(const_cast<char*>(raw));   // caller-owned, per the header
  return out;
}

JNIEXPORT jlong JNICALL
Java_com_despia_base_NativeStore_open(JNIEnv* env, jclass, jstring configJson) {
  const std::string config = jstr(env, configJson);
  return reinterpret_cast<jlong>(despia_base_open(config.c_str()));
}

JNIEXPORT void JNICALL
Java_com_despia_base_NativeStore_close(JNIEnv*, jclass, jlong handle) {
  if (handle) despia_base_close(reinterpret_cast<void*>(handle));
}

JNIEXPORT jint JNICALL
Java_com_despia_base_NativeStore_reopen(JNIEnv*, jclass, jlong handle) {
  if (!handle) return -1;
  return despia_base_reopen(reinterpret_cast<void*>(handle));
}
...
JNIEXPORT jstring JNICALL
Java_com_despia_base_NativeStore_call(JNIEnv* env, jclass, jlong handle,
                                      jstring action, jstring argsJson) {
  if (!handle) return nullptr;
  const std::string name = jstr(env, action);
  const std::string args = jstr(env, argsJson);
  char* raw = despia_base_call(reinterpret_cast<void*>(handle), name.c_str(), args.c_str());
  if (!raw) return nullptr;
  jstring out = env->NewStringUTF(raw);
  despia_base_free(raw);                      // caller-owned, per the header
  return out;
}
...
// despia_base_open answered NULL and left nothing to ask.
JNIEXPORT jstring JNICALL
Java_com_despia_base_NativeStore_lastError(JNIEnv* env, jclass, jlong handle) {
  const char* raw = despia_base_last_error(handle ? reinterpret_cast<void*>(handle) : nullptr);
