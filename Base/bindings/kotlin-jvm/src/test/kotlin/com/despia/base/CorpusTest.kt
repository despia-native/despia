// CorpusTest.kt - the Kotlin runner for the Despia Base conformance corpus.
//
// Copyright Despia. Licensed under the Apache License, Version 2.0.
//
// The same files OpenSource/Base/conformance/run.ts reads, driven through the
...
class CorpusTest {
    ...
    private fun corpusRoot(): File {
        ...
        while (dir != null) {
            for (candidate in listOf(
                File(dir, "OpenSource/Conformance/base"),   // the monorepo
                File(dir, "conformance/base"),              // the published mirror
                File(dir, "../Conformance/base"),
            )) {
    ...
    private fun callBase(h: Harness, action: String, args: Json): Json {
        ...
        return when (action) {
            ...
            else -> throw BaseError("unknown_action", "no base action named $action")
        }
    }
    ...
    private fun embeddingOf(v: Json): Embedding =
        Embedding.from(v) ?: throw BaseError(
            "invalid_embedding",
        ...
        for (model in case["mock"]["models"].fields.values) {
            for (turn in model["turns"].items) {
                for (step in turn.items) {
                    ...
                    try {
                        if (spec["fails"].bool()) throw BaseError("tool_failed", "the tool failed mid-write")
    ...
    private fun runCase(case: Json, fails: MutableList<String>) {
        val dir = Files.createTempDirectory("despia-base-").toFile()
        try {
            Store(dir).use { store ->
                val h = Harness(store)
