// AbiTest.kt - the parts of despia_base.h the corpus does not reach.
//
// Copyright Despia. Licensed under the Apache License, Version 2.0.
//
// The corpus is about what an ACTION means, and it is platform-neutral on
// purpose. The three rules at the top of despia_base.h are about the SEAM -
...
class AbiTest {
    ...
    private fun tempStore(): Pair<Store, java.io.File> {
        val dir = Files.createTempDirectory("despia-base-abi-").toFile()
    ...
    fun `a caller adapts to the reported vector backend instead of assuming one`() {
        ...
        try {
            ...
            if (caps.has("vectors")) {
                ...
                val failure = assertFailsWith<BaseError> {
    ...
    fun `an unknown action is a typed answer, not undefined behaviour`() {
        ...
        try {
            ...
            val plain = assertFailsWith<BaseError> { store.call("vacuum") }
            ...
            val family = assertFailsWith<BaseError> {
    ...
    fun `an unknown config field is ignored and a wrongly typed one is refused`() {
        val dir = Files.createTempDirectory("despia-base-cfg-").toFile()
        try {
    ...
    fun `two handles on one directory see each other's committed writes`() {
        val dir = Files.createTempDirectory("despia-base-wal-").toFile()
    ...
    fun `a closed store refuses further calls instead of using a dangling handle`() {
        ...
        try {
            store.close()
            val failure = assertFailsWith<BaseError> { store.stats() }
    ...
    fun `the three drift reasons stay distinct because the fix differs for each`() {
        ...
        try {
            ...
            val widened = assertFailsWith<BaseError> {
            ...
            val swapped = assertFailsWith<BaseError> {
            ...
            val bumped = assertFailsWith<BaseError> {
            ...
            val unversioned = assertFailsWith<BaseError> {
