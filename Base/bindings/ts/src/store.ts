// store.ts - Despia Base over real SQLite.
//
...
// in OpenSource/Conformance/base assert transactional behaviour, and a
...
export class Base {
      ...
      for (const [op, operand] of Object.entries(condition as Dict)) {
        rows = rows.filter((row) => {
          ...
          switch (op) {
            ...
            default:
              throw new BaseError('unknown_operator', `no such query operator: ${op}`, { op });
          }
        });
      }
    }
  ...
  restore(label: string): Dict {
    const path = this.#snapshotPath(label);
    if (!existsSync(path)) {
      throw new BaseError('unknown_snapshot', `no snapshot labelled ${label}`, { label });
    }
    this.#db.close();
  ...
  indexUpsert(idx: string, key: string, vector: number[], embedding: Embedding, value: Dict): Dict {
    ...
    if (vector.length !== embedding.dimension) {
      throw new BaseError('embedding_mismatch', 'the vector does not match its declared dimension',
  ...
  #requireStore(store: string): void {
    ...
    throw new BaseError('unknown_store', `no store named ${store}`, { store });
  }
  ...
  #requireSameEmbedding(idx: string, stored: Embedding, query: Embedding): void {
    ...
    if (stored.dimension !== query.dimension) {
      throw new BaseError('embedding_mismatch', 'the query embedding has a different dimension',
                          { reason: 'dimension', stored: stored.dimension, query: query.dimension, reindex: idx });
    }
    if (stored.model !== query.model) {
      throw new BaseError('embedding_mismatch', 'the query embedding comes from a different model',
                          { reason: 'model', stored: stored.model, query: query.model, reindex: idx });
    }
    if ((stored.revision ?? null) !== (query.revision ?? null)) {
      throw new BaseError('embedding_mismatch', 'the query embedding is a different revision',
  ...
  #snapshotPath(label: string): string {
    if (!/^[\w.-]+$/.test(label)) {
      throw new BaseError('invalid_label', 'a snapshot label is letters, digits, dot, dash or underscore');
    }
  ...
  #ident(name: string): string {
    if (!/^[A-Za-z_][A-Za-z0-9_]*$/.test(name)) {
      throw new BaseError('invalid_savepoint', `not a valid savepoint name: ${name}`);
    }
    return name;
  }
}
