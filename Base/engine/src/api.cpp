// api.cpp - the C ABI surface declared by despia_base.h.
//
...
// what despia_base.h documents, because there is no behaviour in this file.
//
// The one thing it DOES own is the ownership asymmetry the header states: the
// pointer from despia_base_call is the caller's (malloc'd, freed with
// despia_base_free), the pointer from despia_base_last_error is the handle's.
...
// The last error for calls that have no handle to carry one. despia_base_open
...
uint32_t despia_base_abi_version(void) { return despia::base::kAbiVersion; }

const char* despia_base_capabilities(void) {
  json::Value caps = despia::base::capabilities();
  caps.set("version", DESPIA_BASE_VERSION);
  // Caller-owned, freed with despia_base_free - the header says so, and a
  // static buffer here would be a data race the first time two threads asked.
  return dupString(caps.dump());
}

void despia_base_free(void* p) { std::free(p); }

void* despia_base_open(const char* config_json) {
  json::Value config;
  std::string why;
...
void despia_base_close(void* handle) { delete static_cast<Store*>(handle); }

int despia_base_reopen(void* handle) {
  if (!handle) return -1;
...
char* despia_base_call(void* handle, const char* action, const char* args_json) {
...
const char* despia_base_last_error(void* handle) {
  if (!handle) {
    std::lock_guard<std::mutex> lock(g_globalErrorMu);
