/*
 * despia_base.h - the Despia Base C ABI.
 *
 ...
 * engines):
 *
 *   1. SELF-DESCRIPTION. despia_base_capabilities() reports what THIS build
 ...
 * rather than an implementation detail. despia_base_call() is the funnel, and
 * despia_base_capabilities() is how a caller finds out what it accepts.
 *
...
#ifndef DESPIA_BASE_H
#define DESPIA_BASE_H
 ...
 * OpenSource/Base/tools/symbol_check.rb is what proves it on every build
 * instead of trusting that someone remembered. */
#ifndef DESPIA_BASE_API
#  if defined(_WIN32)
#    if defined(DESPIA_BASE_BUILDING)
#      define DESPIA_BASE_API __declspec(dllexport)
#    else
#      define DESPIA_BASE_API __declspec(dllimport)
#    endif
#  else
#    define DESPIA_BASE_API __attribute__((visibility("default")))
#  endif
#endif
...
DESPIA_BASE_API uint32_t despia_base_abi_version(void);

/* What this build carries, as UTF-8 JSON. The caller owns the returned pointer
 * and frees it with despia_base_free.
 *
...
DESPIA_BASE_API const char* despia_base_capabilities(void);

/* Frees any pointer this ABI returned as caller-owned. Safe on NULL. */
DESPIA_BASE_API void despia_base_free(void* p);
 ...
 * failure is reported through despia_base_last_error(NULL). */
DESPIA_BASE_API void* despia_base_open(const char* config_json);
 ...
 * NULL. */
DESPIA_BASE_API void despia_base_close(void* handle);
 ...
 * (Conformance/base/crud "a committed write survives an abrupt process loss",
...
DESPIA_BASE_API int despia_base_reopen(void* handle);
 ...
 * despia_base_last_error(handle) as { "code", "message", "data" }; `code` is
 ...
 *
 * The whole list is reported by despia_base_capabilities() under `actions`, and
 ...
 * become SQL - Conformance/base/crud "a value never becomes SQL" is the fixture
 * that says so. Handing an app raw SQL through the bus would make that
 * guarantee unenforceable in one line. */
DESPIA_BASE_API char* despia_base_call(void* handle, const char* action,
...
DESPIA_BASE_API const char* despia_base_last_error(void* handle);
 ...
 *   - despia_base_call is SYNCHRONOUS. There is no callback and no worker
 ...
 *     layer does.
 *   - The pointer from despia_base_call is the CALLER's; the pointer from
 *     despia_base_last_error is the HANDLE's. Freeing the second is a
 ...
 *     to own.
 *   - despia_base_close is final. Using a handle after closing it is undefined,
