...
// OpenSource/Conformance/base, and pins BEHAVIOUR across the runtimes. This is
// the layer below: proof that the ABI in despia_base.h does what the header
...
Answer call(void* handle, const std::string& action, const std::string& args) {
  Answer out;
  char* raw = despia_base_call(handle, action.c_str(), args.c_str());
  if (raw) {
    out.ok = true;
    ...
    despia_base_free(raw);
    return out;
  }
  const char* err = despia_base_last_error(handle);
...
void testCapabilities() {
  ...
  check(despia_base_abi_version() == 1, "despia_base_abi_version reports 1");

  const char* first = despia_base_capabilities();
  const char* second = despia_base_capabilities();
  ...
  check(g_caps["abi"].asInt(0) == 1, "capabilities agrees with despia_base_abi_version");
  ...
  despia_base_free(const_cast<char*>(first));
  despia_base_free(const_cast<char*>(second));
  despia_base_free(nullptr);  // documented as safe on NULL
}
...
void testOpenRefusals() {
  ...
  check(despia_base_open("{ this is not json") == nullptr, "malformed config is refused");
  const char* global = despia_base_last_error(nullptr);
  ...
  check(despia_base_open("{\"schema_version\":1}") == nullptr,
        "a config with no directory is refused");
  global = despia_base_last_error(nullptr);
  if (global) {
...
void testErrorOwnership(void* handle) {
  ...
  check(despia_base_call(handle, "still.no.such.action", "{}") == nullptr,
        "a failed call returns NULL");
  check(despia_base_last_error(handle) != nullptr,
        "the failure is readable from the handle afterwards");

  char* good = despia_base_call(handle, "stats", "{}");
  check(good != nullptr, "a successful call returns a result");
  despia_base_free(good);
  check(despia_base_last_error(handle) == nullptr,
        "a successful call clears the last error, so NULL-means-look-here is a rule");

  check(despia_base_call(handle, "", "{}") == nullptr, "an empty action name is refused");
  check(despia_base_call(handle, "get", "{not json") == nullptr,
...
void testReopen(void* handle) {
  ...
  check(despia_base_reopen(handle) == 0, "reopen closes and reopens the same file");
  ...
  check(despia_base_reopen(nullptr) != 0, "reopen on no handle is an error, not a crash");
}
...
int main() {
  std::printf("despia_base abi test\n\n");
  ...
  const fs::path dir = fs::temp_directory_path(ec) /
                       ("despia-base-abi-" + std::to_string(stamp));
  ...
  void* handle = despia_base_open(config.c_str());
  check(handle != nullptr, "the store opens");
  if (!handle) {
    const char* why = despia_base_last_error(nullptr);
  ...
  despia_base_close(handle);
  despia_base_close(nullptr);  // documented as safe on NULL
