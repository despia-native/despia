// run.ts - the TS runner for the Despia Base conformance corpus.
//
// Copyright Despia. Licensed under the Apache License, Version 2.0.
//
// Runs OpenSource/Conformance/base/** against the real SQLite binding in VERIFY
...
import { Base, BaseError } from '../bindings/ts/src/store.ts';
...
function corpusRoot(): string {
  ...
  for (;;) {
    for (const candidate of [
      join(dir, 'OpenSource', 'Conformance', 'base'),   // the monorepo
      join(dir, 'conformance', 'base'),                 // the published mirror
      join(dir, '..', 'Conformance', 'base'),
    ]) {
      if (existsSync(candidate)) return candidate;
    }
    const parent = dirname(dir);
    if (parent === dir) throw new Error('the base corpus was not found');
    dir = parent;
  }
}
...
function callBase(h: Harness, action: string, args: Dict): Dict {
  ...
  switch (action) {
    ...
    default:
      throw new BaseError('unknown_action', `no base action named ${action}`);
  }
}
    ...
    for (const turn of ((model as Dict).turns ?? []) as Dict[][]) {
      for (const step of turn) {
        ...
        if (spec.mutates) {
          ...
          try {
            if (spec.fails) throw new BaseError('tool_failed', 'the tool failed mid-write');
...
function runCase(c: Dict, fail: (f: string) => void): void {
  const dir = mkdtempSync(join(tmpdir(), 'despia-base-'));
  try {
    const base = new Base(dir);
    ...
    if (expect.engineRequests) {
      // The base corpus asserts the TOOL's effect on the store, not the engine
