...
| Core/HealthKit (healthkit) | readhealthkit · writehealthkit | the raw-URL payload grammar harvested as `$tail` rows onto NEW declared actions (`read` gained `query`, `write` gained `body` — each parses its own tail; the alias pre-filter stays LIVE for registry-routed navigations). `readhealthkit` carries the `healthkitResponse` response global; `writehealthkit` carries the NEW `healthkitWriteResponse` acknowledgment (v3 writes reported nothing — additive). Both rows are the FIRST `push` consumers: the module's `unavailable` broadcast re-delivers into the same globals, so a v3 poll settles with `{ error: "healthkit_unavailable" }` instead of timing out. `healthkit://state` is NOT a row (its scheme is the module's own); the pre-filter routes it to `status`, which re-injects `window.healthkitState` the observingHealthKit way |
