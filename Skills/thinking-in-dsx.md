...
> packages is [`porting-a-react-native-library.md`](porting-a-react-native-library.md),
...
| PanResponder / gesture-handler | universal `on:drag*`, `on:pinch`, `on:longpress` | [`custom-ux.md`](custom-ux.md) |
...
- Empty, loading and error states as first-class UI. The bar is
  [`designing-an-app.md`](designing-an-app.md); it applies verbatim.
