...
(function (root, factory) {
  ...
  const BIN_PREC = { "||": 2, "&&": 3, "==": 4, "!=": 4, "===": 4, "!==": 4, "<": 5, ">": 5, "<=": 5, ">=": 5, "+": 6, "-": 6, "*": 7, "/": 7, "%": 7 };
  ...
  function compileNode(node, graph, minPrec, pos, active) {
    ...
    switch (node.kind) {
      ...
      case "object": {
        ...
        const inner = body ? "{ " + body + " }" : "{}";
        // `arrowBody` joins root and target: a bare `{` after `=>` opens a BLOCK on every
        // runtime, so an object return without parentheses is a different program.
        return pos === "root" || pos === "target" || pos === "arrowBody" ? "(" + inner + ")" : inner;
      }
      case "array": return "[" + (node.items || []).map((i) => arg(i)).join(", ") + "]";
      case "arrow": {
        const params = node.params || [];
        const head = params.length === 1 ? params[0] : "(" + params.join(", ") + ")";
        const t = head + " => " + arg(node.body, 2, "arrowBody");
        return wrap(t, 1);
      }
  ...
  function liftTokens(s) {
    ...
    while (i < s.length) {
      ...
      const three = s.slice(i, i + 3), two = s.slice(i, i + 2);
      // The author's SPELLING survives. The evaluator may fold `===` into `==` because JSE
      // has one coercion table for both, but lift feeds compile and compile writes the file:
      // folding here rewrites `a !== b` to `a != b` in a formula nobody edited.
      if (three === "===" || three === "!==") { toks.push({ t: "op", v: three, s: i, e: i + 3 }); i += 3; continue; }
